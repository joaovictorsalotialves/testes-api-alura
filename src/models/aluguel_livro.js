...
class AluguelLivro {
  constructor({ id, livro_id, usuario_id, dias_alugados, alugado, created_at, updated_at }) {
    this.id = null || id
  ...
  async criar() {
    return db('aluguel_livro')
      .insert(this)
      .then(registroCriado => db('aluguel_livro').where('id', registroCriado[0]))
  ...
  static async excluir(id) {
    ...
    await db('aluguel_livro').where({ id }).del()
  }
