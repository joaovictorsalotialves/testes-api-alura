...
class LivroImagem {
  constructor({ id, livro_id, mimetype, filename, size, base64, created_at, updated_at }) {
    this.id = null || id
  ...
  async criar() {
    return db('livros_imagens')
      .insert(this)
      .then(registroCriado => db('livros_imagens').where('id', registroCriado[0]))
  ...
  static async excluir(id) {
    ...
    await db('livros_imagens').where({ id }).del()
  }
